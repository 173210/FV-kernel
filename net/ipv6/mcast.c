...
#ifdef CONFIG_TOSHIBA_IPTV_MLDV2_FOR_STB
int ipv6_sock_mc_join_ssm(struct sock *sk, int ifindex, struct in6_addr *addr,
			  struct in6_addr *addr2)
{
	struct net_device *dev = NULL;
	struct ipv6_mc_socklist *mc_lst;
	...
	err = ipv6_dev_mc_inc_ssm(dev, addr, addr2);
...
#ifdef CONFIG_TOSHIBA_IPTV_MLDV2_FOR_STB
int ipv6_dev_mc_inc_ssm(struct net_device *dev, struct in6_addr *addr,
			struct in6_addr *addr2)
{
	struct ifmcaddr6 *mc;
	struct inet6_dev *idev;
	...
	for (mc = idev->mc_list; mc; mc = mc->next) {
		if (ipv6_addr_equal(&mc->mca_addr, addr)) {
			mc->mca_users++;
			write_unlock_bh(&idev->lock);
			ip6_mc_add_src_ssm(idev, &mc->mca_addr, MCAST_INCLUDE, 0,
				NULL, 0);
			in6_dev_put(idev);
			return 0;
		}
