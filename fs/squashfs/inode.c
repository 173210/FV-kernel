...
static int squashfs_linear_get_sb(struct file_system_type *fs_type, int flags, const char *dev_name, void *data, struct vfsmount *mnt);
static void *squashfs_linear_read(struct super_block *sb, unsigned int offset, unsigned int len);
...
#ifdef CONFIG_SQUASHFS_LINEAR
static int get_block_length_linear(struct super_block *s,
				int *cur_index, int *offset, int *c_byte)
{
	struct squashfs_sb_info *msblk = s->s_fs_info;
	unsigned short temp;
	char *data = squashfs_linear_read(s,*cur_index * msblk->devblksize,msblk->devblksize);
	...
	*offset += 2;
	...
	if (SQUASHFS_CHECK_DATA(msblk->sblk.flags)) {
		if (*offset == msblk->devblksize) {
			++(*cur_index);
			*offset = 0;
			data = squashfs_linear_read(s,*cur_index * msblk->devblksize,msblk->devblksize);
		}
		if (*((unsigned char *) (data + *offset)) !=
						SQUASHFS_MARKER_BYTE) {
			ERROR("Metadata block marker corrupt @ %x\n",
						*cur_index);
			goto out;
...
static void *squashfs_linear_read(struct super_block *sb, unsigned int offset, unsigned int len)
{
	...
	return (void*)(msblk->linear_virt_addr + offset);
}
#endif
			...
			long long *next_index, int srclength)
{
	...
	if (c_byte) {
		...
		if(LINEAR(msblk)){
			if(compressed)
				c_buffer = squashfs_linear_read(s, index, c_byte);
			else{
				c_buffer = buffer;
				memcpy(buffer, squashfs_linear_read(s, index, c_byte), c_byte);
				bytes = c_byte;
			}
		} else {
#endif
...
#ifdef CONFIG_SQUASHFS_LINEAR
		if(LINEAR(msblk)){
			if(!get_block_length_linear(s, &cur_index, &offset, &c_byte))
				goto read_failure;
		} else {
#endif
		...
		if(LINEAR(msblk)){
			if(compressed)
				c_buffer = squashfs_linear_read(s, cur_index * msblk->devblksize + offset, c_byte);
			else {
				c_buffer = buffer;
				memcpy(buffer, squashfs_linear_read(s, cur_index * msblk->devblksize + offset, c_byte), c_byte);
				bytes = c_byte;
			}
		} else {
#endif
...
#if defined(CONFIG_SQUASHFS_LINEAR) || defined(CONFIG_SQUASHFS_DISABLE_READAHEAD)
...
#ifdef CONFIG_SQUASHFS_DISABLE_READAHEAD
	i->i_mapping->backing_dev_info = &squashfs_backing_dev_info;
#else
#ifdef CONFIG_SQUASHFS_LINEAR
	if (LINEAR(msblk))
		i->i_mapping->backing_dev_info = &squashfs_backing_dev_info;
...
static int squashfs_fill_super(struct super_block *s, void *data, int silent)
{
	...
	if (!data || !(p = strstr((char *)data, "physaddr="))) {
		if (!s->s_bdev)	/* avoid crash */
			goto failed_mount;
		goto read;
	}
	msblk->linear_phys_addr = simple_strtoul(p + 9, NULL, 0);
	if (msblk->linear_phys_addr & (PAGE_SIZE-1)) {
		printk(KERN_ERR "squashfs: physical address 0x%lx for"
			" linear squashfs isn't aligned to a page boundary\n",
		       msblk->linear_phys_addr);
		goto failed_mount;
	}
	if (msblk->linear_phys_addr == 0) {
		printk(KERN_ERR "squshfs: physical address for "
			"linear squashfs image can't be 0\n");
		goto failed_mount;
	}
	printk(KERN_INFO "squashfs: checking physical address 0x%lx for"
	...
	if (!msblk->linear_virt_addr) {
		printk(KERN_ERR "squashfs: ioremap of the linear squashfs image failed\n");
		goto failed_mount;
	}
read:
	if(LINEAR(msblk))
...
#ifdef CONFIG_SQUASHFS_LINEAR
			if(LINEAR(msblk))
				WARNING("Mounting a different endian SQUASHFS filesystem on 0x%08lx\n", msblk->linear_phys_addr);
			else
#endif
...
#ifdef CONFIG_SQUASHFS_LINEAR
			if(LINEAR(msblk))
				SERROR("Can't find a SQUASHFS superblock on 0x%lx\n", msblk->linear_phys_addr);
			else
#endif
...
#ifdef CONFIG_SQUASHFS_LINEAR
	if (LINEAR(msblk)) {
		int size = ALIGN(sblk->bytes_used,PAGE_SIZE);
		/* Remap the whole filesystem now */
		iounmap(msblk->linear_virt_addr);
		printk(KERN_INFO "squashfs: linear squashfs image appears to be %u KB in size\n",
		       size >> 10);
...
#ifdef CONFIG_SQUASHFS_LINEAR
	if((err = register_filesystem(&squashfs_linear_fs_type))) {
		destroy_inodecache();
		return err;
	}
