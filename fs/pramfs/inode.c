...
pram_truncate_blocks(struct inode * inode, int first_trunc_block)
{
	...
	if (first_blocknr == 0) {
		...
		pram_unlock_inode(pi);
	} else if ((inode->i_size & (sb->s_blocksize - 1)) != 0) {
		int offset;
		pram_off_t block;
		void *bp;

		block = pram_find_data_block(inode, first_blocknr - 1);
		bp = pram_get_block(sb, block);
		if (bp) {
			offset = (inode->i_size & (sb->s_blocksize - 1));
			memset((char *)bp + offset, 0, sb->s_blocksize - offset);
		}
	}
	pram_lock_inode(pi);
	pi->i_blocks = inode->i_blocks;
...
static int pram_writepage(struct page *page, struct writeback_control *wbc)
{
	...
	if (page->index > end_index) {
		printk("try to write beyond end of the file.\n");
		return -EIO;
	}
	...
	do {
		if (!buffer_mapped(bh) && buffer_dirty(bh)) {
			if (pram_get_and_update_block(inode, iblock, bh, 1)) {
				printk(KERN_ERR "get_block error in %s\n",
				       __func__);
				continue;
			}
			if (buffer_new(bh))
		...
		if (test_clear_buffer_dirty(bh)) {
			...
			lock_kernel();
			block = pram_find_data_block(inode, iblock);
			BUG_ON(bh->b_blocknr != block);

			bp = pram_get_block(sb, block);
			BUG_ON(buffer_new(bh));

			flush_dcache_page(bh->b_page);
			memcpy(bp, bh->b_data, blocksize);
			unlock_kernel();
		}
		unlock_buffer(bh);
