...
static int prepare_write_slow(struct file *f, struct page *page,
 ...
 */
static int ubifs_prepare_write(struct file *f, struct page *page,
			       unsigned offset, unsigned to)
{
	struct inode *inode = page->mapping->host;
	struct ubifs_info *c = inode->i_sb->s_fs_info;
	...
	if (unlikely(err)) {
		...
		return prepare_write_slow(f, page, offset, to);
	}
...
static int ubifs_commit_write(struct file *f, struct page *page, unsigned offset,
			      unsigned to)
{
	struct inode *inode = page->mapping->host;
	struct ubifs_inode *ui = ubifs_inode(inode);
	struct ubifs_info *c = inode->i_sb->s_fs_info;
	loff_t pos = ((loff_t)page->index << PAGE_CACHE_SHIFT) + offset;
	unsigned len = to - offset;	/* "bytes" in generic_file_buffered_write() */
	loff_t end_pos = pos + len;
	int appending = !!(end_pos > inode->i_size);

	dbg_gen("ino %lu, pos %llu, pg %lu, len %d, i_size %lld",
		inode->i_ino, pos, page->index, len, inode->i_size);
	...
	return 0;	/* "copied" is used in generic_file_buffered_write() */
}
