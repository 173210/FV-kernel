		...
		int high_priority)
{
	...
	char data_head[USER_TRIG_STR_SIZE];
...
#ifdef CONFIG_LTT_USEREVENT_TRACECONTROL
	if (ltt_traces.num_start_trig_u_traces > 0 || ltt_traces.num_stop_trig_u_traces > 0) {
		if (__copy_from_user_inatomic(data_head, data, data_size < USER_TRIG_STR_SIZE ? data_size : USER_TRIG_STR_SIZE)) {
			return -EFAULT;	/* Data is garbage in the slot */
		}

		data_head[USER_TRIG_STR_SIZE - 1] = '\0';
	}

	if (ltt_traces.num_start_trig_u_traces > 0) {
		(void)ltt_trigger_control_start_u(data_head);
	}
#endif //CONFIG_LTT_USEREVENT_TRACECONTROL
...
#ifdef CONFIG_LTT_USEREVENT_TRACECONTROL
	if (ltt_traces.num_stop_trig_u_traces > 0) {
		(void)ltt_trigger_control_stop_u(data_head);
	}
#endif //CONFIG_LTT_USEREVENT_TRACECONTROL
