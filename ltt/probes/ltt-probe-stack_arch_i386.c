...
/* norm : inline */
static noinline unsigned long dump_context_stack(struct thread_info *tinfo,
...
static noinline void dump_fp_process_stack(
		struct pt_regs *regs,
		char *buffer,
		char **str,
		long *len)
...
static noinline void dump_nofp_process_stack(
		struct pt_regs *regs,
		char *buffer,
		char **str,
		long *len)
...
static noinline char *ltt_serialize_process_stack(char *buffer, char *str,
	struct ltt_serialize_closure *closure,
	int align, const char *fmt, va_list *args)
{
	struct pt_regs *regs;
