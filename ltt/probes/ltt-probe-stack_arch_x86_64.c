...
static noinline unsigned long *trace_in_exception_stack(unsigned cpu,
					unsigned long stack,
...
static noinline void dump_trace(unsigned long *stack,
		char *buffer,
		char **str,
		long *len)
{
...
static noinline char *ltt_serialize_kernel_stack(char *buffer, char *str,
...
static noinline void dump_fp_process32_stack(
		...
		long *len)
{
	...
	regs = (struct user_regs_struct32 *)real_regs;

	if (buffer)
		*dest = PTR_LOW(regs->eip);
	else
		(*len)++;
	dest++;

	/* Start at the topmost ebp */
	iter = (uint32_t*)PTR_LOW(regs->ebp);
...
static noinline void dump_nofp_process32_stack(
		struct pt_regs *real_regs,
		char *buffer,
		char **str,
		long *len)
{
	...
	prev_iter = beg_iter = iter = (uint32_t*)PTR_LOW(regs->esp);
...
static noinline void dump_fp_process64_stack(
		struct pt_regs *regs,
		char *buffer,
		char **str,
		long *len)
...
static noinline void dump_nofp_process64_stack(
		struct pt_regs *regs,
		char *buffer,
		char **str,
		long *len)
...
static noinline char *ltt_serialize_process32_stack(char *buffer, char *str,
	struct ltt_serialize_closure *closure,
	int align, const char *fmt, va_list *args)
{
	struct pt_regs *regs;
...
static noinline char *ltt_serialize_process64_stack(char *buffer, char *str,
	struct ltt_serialize_closure *closure,
	int align, const char *fmt, va_list *args)
{
	struct pt_regs *regs;
