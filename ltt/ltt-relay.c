			...
			u64 tsc, unsigned int offset, unsigned int subbuf_idx)
{
	...
	if (channel->buf[buf->cpu].first_subbuffer) {
		/* the first block */
		channel->buf[buf->cpu].first_subbuffer = 0;
		printk(KERN_DEBUG "%s: FIRST_BLOCK: events_lost = %lu, subbuf_corrupt = %lu\n", channel->channel_name, 
		       local_read(&channel->buf[buf->cpu].events_lost),
		       local_read(&channel->buf[buf->cpu].corrupted_subbuffers));
	} else {
		/* the second block and later */
		header->trace.freq_scale = 0;
		header->trace.events_lost = local_read(&channel->buf[buf->cpu].events_lost);
		header->trace.subbuf_corrupt = local_read(&channel->buf[buf->cpu].corrupted_subbuffers);
		printk(KERN_DEBUG "%s: events_lost = %u, subbuf_corrupt = %u\n", channel->channel_name, header->trace.events_lost, header->trace.subbuf_corrupt);
	}
}
	...
	unsigned n_subbufs)
{
	...
	ltt_chan->buf[cpu].first_subbuffer = 1;
