...
static void __ltt_trace_destroy(struct ltt_trace_struct	*trace)
{
	...
	if (trace->start_trig_u[0] != '\0') {
		ltt_traces.num_start_trig_u_traces--;
		trace->start_trig_u[0] = '\0';
	}
	if (trace->stop_trig_u[0] != '\0') {
		ltt_traces.num_stop_trig_u_traces--;
		trace->stop_trig_u[0] = '\0';
	}
...
static void full_heartbeat_ipi(void *info)
{
	...
	#define HB_FULL_FLAGS \
		(_MF_DEFAULT | ltt_flag_mask(LTT_FLAG_TRACE) \
			| ltt_flag_mask(LTT_FLAG_CHANNEL)\
	...
	_MARK(HB_FULL_FLAGS,
		...
		ltt_get_timestamp64());
	_MARK(_MF_DEFAULT
		| ltt_flag_mask(LTT_FLAG_TRACE)
		| ltt_flag_mask(LTT_FLAG_COMPACT)
		| ltt_flag_mask(LTT_FLAG_FORCE),
...
#ifdef CONFIG_LTT_USEREVENT_TRACECONTROL
static int ltt_trace_set_start_trig_user(char *trace_name, char *user_trig_str)
{
	int err = 0;
	struct ltt_trace_struct* trace;
	...
	if (trace->start_trig_u[0] == '\0' && user_trig_str[0] != '\0') {
		ltt_traces.num_start_trig_u_traces++;
	} else if (trace->start_trig_u[0] != '\0' && user_trig_str[0] == '\0') {
		ltt_traces.num_start_trig_u_traces--;
	}
	strncpy(trace->start_trig_u, user_trig_str, USER_TRIG_STR_SIZE);
	trace->start_trig_u[USER_TRIG_STR_SIZE-1] = '\0';
...
static int ltt_trace_set_stop_trig_user(char *trace_name, char *user_trig_str)
{
	int err = 0;
	struct ltt_trace_struct* trace;
	...
	if (trace->stop_trig_u[0] == '\0' && user_trig_str[0] != '\0') {
		ltt_traces.num_stop_trig_u_traces++;
	} else if (trace->stop_trig_u[0] != '\0' && user_trig_str[0] == '\0') {
		ltt_traces.num_stop_trig_u_traces--;
	}
	strncpy(trace->stop_trig_u, user_trig_str, USER_TRIG_STR_SIZE);
	trace->stop_trig_u[USER_TRIG_STR_SIZE-1] = '\0';
		...
		union ltt_control_args args)
{
	...
	switch (msg) {
		...
		case LTT_CONTROL_DESTROY_TRACE:
			...
			break;
#ifdef CONFIG_LTT_USEREVENT_TRACECONTROL
		case LTT_CONTROL_SET_START_TRIG_USER:
			printk(KERN_DEBUG "Set start trigger user string '%s' to trace %s\n", args.user_trig_str, trace_name);
			err = ltt_trace_set_start_trig_user(trace_name, args.user_trig_str);
			break;
		case LTT_CONTROL_SET_STOP_TRIG_USER:
			printk(KERN_DEBUG "Set stop trigger user string '%s' to trace %s\n", args.user_trig_str, trace_name);
			err = ltt_trace_set_stop_trig_user(trace_name, args.user_trig_str);
			break;
#endif //CONFIG_LTT_USEREVENT_TRACECONTROL
	}
	return err;
...
EXPORT_SYMBOL_GPL(ltt_statedump_process);

#ifdef CONFIG_LTT_USEREVENT_TRACECONTROL
int ltt_trigger_control_start_u(char *user_trig_str)
{
	int err = 0;
	struct ltt_trace_struct* trace;
...
retry:
	...
	list_for_each_entry_rcu(trace, &ltt_traces.head, list) {
		if (trace->active ||
		    strncmp(trace->start_trig_u, user_trig_str, USER_TRIG_STR_SIZE) != 0)
			continue;

		/* found start trigger */
		printk(KERN_DEBUG "Start tracing %s by trigger string %s\n", trace->trace_name, trace->start_trig_u);
...
EXPORT_SYMBOL_GPL(ltt_trigger_control_start_u);

int ltt_trigger_control_stop_u(char *user_trig_str)
{
	int err = 0;
	struct ltt_trace_struct* trace;
	...
	list_for_each_entry_rcu(trace, &ltt_traces.head, list) {
		if (!trace->active ||
		    strncmp(trace->stop_trig_u, user_trig_str, USER_TRIG_STR_SIZE) != 0)
			continue;

		/* found stop trigger */
		printk(KERN_DEBUG "Stop tracing %s by trigger string %s\n", trace->trace_name, trace->stop_trig_u);
...
EXPORT_SYMBOL_GPL(ltt_trigger_control_stop_u);
#endif //CONFIG_LTT_USEREVENT_TRACECONTROL
